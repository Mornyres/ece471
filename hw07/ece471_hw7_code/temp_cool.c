...
double myAverage(double* inputData, int size);
int shiftArray(double* inputData, double insertValue, int sampleSize);

int main(int argc, char **argv) {
	
	/* something cool vars */
	double c1Array[AVGSIZE];	// will preserve only the past AVGSIZE values
...
are bits 9 and 8, and the third byte is the bottom 8 bits of the result */	
		...
		/* something cool function call */
		/* add C1 to running array */
		shiftArray(c1Array, output_C1, AVGSIZE);
		output_C1 = myAverage(c1Array,AVGSIZE);	
...
// loosely speaking, sum of previous n samples over n (in this case n = size)
double myAverage(double* inputData, int size){
...
int shiftArray(double* inputData, double insertValue, int sampleSize){
	int i;
	printf("Adding %.2lf to running mean\r\n", insertValue);
	// shift everything down
	for (i=0;i<sampleSize-1;i++){
		inputData[i] = inputData[i+1];
	}
	// insert new value
	inputData[sampleSize-1] = insertValue;
	
	for (i=0;i<sampleSize;i++){
		if(DEBUGMODE==1){
			printf("%d.%2lf",i,inputData[i]);
		}
	}
